 ...
 **/
public class Form extends JComponent implements DragGestureListener, 
        ...
        MouseMotionListener // For processing mouse drags
{
	...
	ArrayList<DecartsLeaf> leafs = new ArrayList<DecartsLeaf>();
    // A list of Scribbles to draw
    DecartsLeaf currentScribble; // The scribble in progress
    DecartsLeaf beingDragged; // The scribble being dragged
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g2.setStroke(new SloppyStroke(1)); // Specify wide lines

        int numScribbles = leafs.size();
        for (int i = 0; i < numScribbles; i++)
        {
            DecartsLeaf s = leafs.get(i);
    ...
    public void mousePressed(MouseEvent e)
    {
        if (dragMode)
            return;
        currentScribble = new DecartsLeaf(e.getX() - 100, e.getY() - 100, 200, 200, 30);
    ...
    public void dragGestureRecognized(DragGestureEvent e)
    {
        ...
        { // Loop through the scribbles
            ...
            if (s.intersects(r))
            {
                ...
                DecartsLeaf dragScribble = (DecartsLeaf) s.clone();
    ...
    {
        if (e.isDataFlavorSupported(DecartsLeaf.decDataFlavor) || e.isDataFlavorSupported(DataFlavor.stringFlavor))
        {
    ...
    public void drop(DropTargetDropEvent e)
    {
        ...
        DecartsLeaf droppedScribble; // This will hold the Scribble object

        // First, try to get the data directly as a scribble object
        try
        {
            droppedScribble = (DecartsLeaf) t.getTransferData(DecartsLeaf.decDataFlavor);
        } catch (Exception ex)
        { // unsupported flavor, IO exception, etc.
          // If that doesn't work, try to get it as a String and parse it
            try
            {
                ...
                droppedScribble = DecartsLeaf.getFromString(s);
            } catch (Exception ex2)
            {
    ...
    public static void main(String[] args)
    {
        ...
        final Form scribblePane = new Form();
